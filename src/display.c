...
bool initialize_window(void) {
  ...
  window =
      SDL_CreateWindow(NULL, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                       display_mode.w, display_mode.h, SDL_WINDOW_BORDERLESS);
  if (!window) {
 ...
 * It takes side length (the longest side) to produce the ratio needed to
 * increment Using the draw_pixel function it places a color on the pixel map
...
void draw_line(int x0, int y0, int x1, int y1, uint32_t color) {
  ...
  // Here we calculate how much we need to run, we use abs() because we don't
  // want negatives
...
/*
 * #   --> Comments so (skip)
 * v   --> vertices information (need) x,y,z values plus optional
 * vt  --> Texuture coordinates (bypass)
 * vn  --> Unit Vectors (ignore)
 * vp  --> Space Vertices also (ignore)
 * f   --> Each Triangle Faces of Mesh
 *
 *
 * TODO: Need v and f
 * for f we just need the first value
 * */
  ...
  while (fgets(line, 1024, file)) {
    ...
    if (!strncmp(line, "f ", 2)) {
      ...
      sscanf(line, "f %d/%d/%d %d/%d/%d %d/%d/%d", &vertex_indices[0],
             &texture_indicies[0], &normal_indicies[0], &vertex_indices[1],
             &texture_indicies[1], &normal_indicies[1], &vertex_indices[2],
             &texture_indicies[2], &normal_indicies[2]);
