...
void setup(void) {
  ...
  // Loads Cube Values to Mesh Datastructure
  load_obj_file("./assets/f22.obj");
}

/*
 * Processes Input Commands by first creating a event struct and then receives
 * what that event struct has recieved by polling the event. PollEvent is a way
 * to recieve system inputs
 * */
...
void update(void) {
  ...
  for (int i = 0; i < num_faces; i++) {
    ...
    for (int j = 0; j < 3; j++) {
      ...
      transform_vertex = vec3_rotate_y(transform_vertex, mesh.mesh_rotation.x);
      transform_vertex = vec3_rotate_x(transform_vertex, mesh.mesh_rotation.y);
      transform_vertex = vec3_rotate_z(transform_vertex, mesh.mesh_rotation.z);
...
void render(void) {
  ...
  for (int i = 0; i < num_of_triangles; i++) {
    ...
    draw_triangle(triangle.points[0].x, triangle.points[0].y,
                  ...
                  triangle.points[2].x, triangle.points[2].y, 0xFFFFC0CB);
  }

  // Clear the array after every render
...
// Frees memory that was dynamically allocated by the program
